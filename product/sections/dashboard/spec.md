...
The Dashboard is the operator's home — a card grid of all active events, each surfacing where it is in the 12-step pipeline at a glance. It's optimized for the question "what needs my attention?" so the DJ can spot a stuck job, a stale build, or a sync that hasn't run yet without opening every event.

## User Flows

- Operator lands on `/`, sees every active event as a card and can identify any that need action (failed job, stale build, sync pending) at a glance.
...
- **Card contents (top to bottom):**
  ...
  - Current step badge (e.g. "Analyze Mood — step 7 of 12") with a thin segmented progress bar showing completed / current / remaining steps
  - Track counts row: `total / matched / tagged / synced` in JetBrains Mono with subtle dividers
